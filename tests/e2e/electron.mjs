...
try {
  ...
  const expected = [
    ...
    "deleteView", "downloadAttachment", "getAuthStatus", "getMail", "getSettings", "listDrafts", "listEvents", "listFolders",
  ...
  // ---------- 日历（真实 IPC：mail:list-events） ----------
  await window.click('[data-nav="calendar"]');
  const calOk = await window
    .waitForSelector('[data-testid="calendar-grid"]', { timeout: 10000 })
    .then(() => true)
    .catch(() => false);
  check('日历页在真实 IPC 下可用（月视图渲染）', calOk);
  const calMonth = await window.$eval('[data-testid="calendar-month"]', (el) => el.textContent || '');
  check('日历显示年月标题', /\d{4} 年 \d{1,2} 月/.test(calMonth), calMonth.trim());
  const calCells = await window.locator('.cal-cell').count();
  check('日历月视图格子数量合理', calCells >= 28 && calCells <= 42, `实际 ${calCells}`);
  await window.click('[data-nav="inbox"]');
  await window.waitForSelector('.mail-list .mail-item', { timeout: 10000 });
