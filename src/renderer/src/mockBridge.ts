...
最相关的是「${top[0].subject}」。` : ''}`
    ...
    /**
     * 日历（mock）：没有真实索引卡片，所以按邮件时间**确定性地**造几个事件，
     * 让渲染层 E2E 能验证月视图/某天列表/跳转原文；事件都落在邮件当天前后，
     * 保证「当月」一定有内容（E2E 会点开当月某天）。
     */
    async listEvents(args: { from: number; to: number }) {
      const list = await this.listMails({ limit: 1000, offset: 0 })
      const out: import('../../shared/calendar').CalendarEvent[] = []
      const DAY = 86_400_000
      list.forEach((m, i) => {
        if (i % 3 !== 0) return // 每 3 封造一条，避免太密
        const dueTs = m.dateTs + 2 * DAY
        const events = mailToEvents({
          ...
          dueTs,
          card: i % 6 === 0 ? `[TYPE] 考试
[FACTS] 相关安排请见 通知，模拟事件 ${i}` : null
        })
        for (const e of events) if (e.ts >= args.from && e.ts <= args.to) out.push(e)
      })
